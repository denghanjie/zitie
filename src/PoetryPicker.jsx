...
export default function PoetryPicker({onApply}){
 ...
 async function search(ai=false){
  ...
  try{
   ...
   }else found=searchWorks(lib.works,lib.simplify(query));
 ...
 return <section className="poetry-picker" aria-label="查找诗词全文">
  ...
  {!!results.length&&<ul className="poem-results">{results.map(w=><li key={w.id}><button type="button" aria-pressed={selected?.id===w.id} onClick={()=>{setSelected(w);setExcerpt(w.text)}}><strong>{w.title}</strong><span>{w.author} · {w.collection}</span><small>{w.text.split('\n')[0]}</small></button></li>)}</ul>}
  {selected&&<div className="poem-detail"><h3>{selected.title}</h3><p>{selected.author} · {selected.collection}</p><a href={selected.sourceUrl} target="_blank" rel="noreferrer">查看原始来源 ↗</a>{selected.correctionNote&&<p className="source-note">用字校订：{selected.correctionNote}。<a href={selected.correctionSourceUrl} target="_blank" rel="noreferrer">查看校订依据 ↗</a></p>}<small>原始文件第 {selected.sourceIndex} 条记录；繁体转为简体。可能存在异文或录入差异，填入前请核对。分阕可在填入后用空行调整。</small><label htmlFor="poem-fulltext">全文预览（可编辑或摘选段落）</label><textarea id="poem-fulltext" value={excerpt} onChange={e=>setExcerpt(e.target.value)}/><p className={length>3000?'length-warning':''}>{length} / 3000 字{length>3000?' · 全文超出限制，请先删减或摘选段落，系统不会截断正文。':''}{excerpt!==selected.text?' · 已编辑':''}</p><button type="button" className="primary" disabled={!excerpt.trim()||length>3000} onClick={()=>onApply({...selected,text:excerpt})}>用此正文替换练习内容</button></div>}
 </section>
}
