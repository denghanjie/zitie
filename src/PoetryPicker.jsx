...
export default function PoetryPicker({onApply}){
 ...
 async function search(ai=false){
  ...
  try{
   const lib=await getLibrary();let found=searchWorks(lib.works,lib.simplify(query),lib.works.length);let usedAI=false;
   if(ai&&!found.length){
    usedAI=true;
   ...
   setResults(found);setMessage(found.length?`${usedAI?'AI 提供检索线索，已在诗文与教材库找到':ai?'已直接匹配原句或篇名，无需 AI，找到':'找到'} ${found.length}${found.length===30?' 条候选（最多展示 30 条）':' 条候选'}。请选择作者和首句吻合的作品。`:`当前收录的 ${lib.works.length} 条记录中没有匹配结果。${ai?'未生成或补写原文。':'可换一个标题、作者或原句，也可以尝试 AI 帮找。'}`);
