...
function Settings({draft,setDraft,busy,generate,trial,trialBusy}){
 ...
 <button className="trial-button" type="button" disabled={trialBusy||busy} onClick={()=>trial('ink')}>{trialBusy?'正在制作试印页…':'下载深浅与空心对比试印页'}</button><button className="trial-button" style={{marginTop:8}} type="button" disabled={trialBusy||busy} onClick={()=>trial('size')}>下载字号大小对比试印页</button><p className="source-note">觉得字大可选「较小 · 65%」，字格不变。圆珠笔建议先试浅色。逐字练习首格和笔顺保留清晰示范；深浅与空心应用于描写范字。</p>
