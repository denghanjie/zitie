...
function getDraft(){try{const d={...example,...JSON.parse(localStorage.getItem('yizi-draft')||'{}')};if(!d.practiceProfile)Object.assign(d,{practiceProfile:'hardpen',font:'wenkai',fontSize:'normal',ink:'medium',traceStyle:'solid'});return migrateLibraryDraft({...d,font:normalizeTypeface(d.font),fontSize:normalizeFontSize(d.fontSize),ink:normalizeInk(d.ink),traceStyle:d.traceStyle==='outline'?'outline':'solid'})}catch{return {...example,practiceProfile:'hardpen',font:'wenkai',fontSize:'normal',ink:'medium',traceStyle:'solid'}}}
function Settings({draft,setDraft,busy,generate,trial,trialBusy}){
 ...
 <button className="trial-button" type="button" disabled={trialBusy||busy} onClick={()=>trial('reference')}>下载范例尺寸试印页（15 毫米字格）</button>
 <details className="print-comparisons"><summary>更多试印对比</summary><button className="trial-button" type="button" disabled={trialBusy||busy} onClick={()=>trial('ink')}>{trialBusy?'正在制作试印页…':'下载深浅与空心对比试印页'}</button><button className="trial-button" style={{marginTop:8}} type="button" disabled={trialBusy||busy} onClick={()=>trial('size')}>下载字号大小对比试印页</button></details><p className="source-note">觉得字大可选「较小 · 65%」，字格不变。圆珠笔建议先试浅色。逐字练习首格和笔顺保留清晰示范；深浅与空心应用于描写范字。</p>
