...
无需注册，无需配置 AI 服务或 API Key。一次最多输入 3000 个字符。逐字模式按原文顺序保留重复汉字。

## 使用方法

1. 打开[在线网站](https://zitie.denghanjie.vip/)，粘贴想练习的文字。
2. 选择「逐字练习」或「整篇临摹」。
3. 设置字格和字帖标题，点击「生成字帖」。
...
技术栈：**React、Vite、SVG、jsPDF**。应用不需要数据库或后端 API。GitHub Actions 在推送和 Pull Request 时执行测试与构建，不会自动发布到服务器。

## 数据与隐私

文字排版和 PDF 生成在浏览器内完成，输入内容与设置存储在当前浏览器的 `localStorage`。应用没有上传整篇练习内容的接口，也没有账号系统或第三方统计脚本。
...
- 自动排版不会识别作者、补全作品或校正文句；标题和正文需要自行输入。
...
覆盖《如梦令》《水调歌头》《梦游天姥吟留别》的三种断句方式，以及内容完整性、长句折行、分阕分页、标点避头、3000 字输入、普通文章和逐字练习。
